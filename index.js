...
const API_KEY =
  "live_HG4AJNZqBzGY01gDEqT1q95iM6l2HiJgrjm8qRSP16EK0jzKjaL6JPSkk1LBDUn1";
...
async function generateCarousel(id) {
  ...
  let selectedBreedImages = await fetch(
    `https://api.thecatapi.com/v1/images/search?limit=100&breed_ids=${selectedBreedId}`,
    {
      method: "GET",
      withCredentials: true,
      headers: {
  ...
  for (let selectedBreedImage of selectedBreedImages) {
    ...
    let newClone = window.createCarouselItem(
      selectedBreedImage.url,
      `${selectedBreed.name} Photo# ${n}`,
      selectedBreedImage.id
    );
...
axios.interceptors.request.use(
  (config) => {
    ...
    console.log(
      `Request started: ${config.url} at ${config.metadata.startTime}`
    );
    return config;
  },
  (error) => {
...
axios.interceptors.response.use(
  (response) => {
    ...
    console.log(
      `Response received from ${response.config.url} at ${endTime} in ${timeTaken}ms`
    );
    return response;
  },
  (error) => {
...
getFavouritesBtn.addEventListener("click", async function () {
  ...
  for (let favourite of favourites) {
    ...
    let newClone = window.createCarouselItem(
      favourite.image.url,
      `${favourite.name} PhotoId ${favourite.image.id}`,
      favourite.image.id
    );
    //  *  - Append each of these new elements to the carousel.
    window.appendCarousel(newClone);
